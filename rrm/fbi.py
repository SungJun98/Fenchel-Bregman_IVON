"""Recursive-reasoning FBI objective and failure state."""
...
class FBI(FB):
    """Recursive-reasoning FBI objective with a smooth failure coordinate."""
    ...
    def objective(
        ...
        *,
        ...
        weight = cls.finite_k_weight(estimate, int(config.candidate_count))
...
def fenchel_bregman_objective(
    ...
    *,
    ...
    """Keep the published RRM objective API available."""
    return FBI.objective(per_example_loss, failure_probability, config=config)
...
class FailureProbabilityTracker(FB):
    ...
    def weights_for(self, group_ids: Tensor, *, candidate_count: int) -> Tensor:
        ...
        return self.finite_k_weight(values, int(candidate_count))
    ...
    def update(
        ...
        *,
        ...
        for index, value in zip(indices.tolist(), observed, strict=True):
            self.ema_failure(self.values[index], value, self.beta)
            self.visit_counts[index].add_(1)
            for candidate_count, cached in self._weight_cache.items():
                cached[index] = self.finite_k_weight(self.values[index], candidate_count)
