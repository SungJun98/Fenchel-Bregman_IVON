...
import recipe.mmpo.core_algos  # noqa: F401
from auto_lm.fbi import FBI, stable_problem_ids, training_decoding_seeds, validate_fb_training_contract


_SAMPLER_GENERATOR_STATE = "__fb_sampler_generator_state__"


def _saved_sampler_generator_state(dataloader_state: dict) -> torch.Tensor:
    if _SAMPLER_GENERATOR_STATE in dataloader_state:
        return dataloader_state[_SAMPLER_GENERATOR_STATE]
    found = []
    stack = [dataloader_state]
    while stack:
        value = stack.pop()
        if isinstance(value, dict):
            found.extend(item for key, item in value.items() if key == "generator" and isinstance(item, torch.Tensor))
            stack.extend(item for item in value.values() if isinstance(item, dict))
    if len(found) != 1:
        raise ValueError("checkpoint does not contain exactly one sampler generator state")
    return found[0]
...
def compute_advantage(
    ...
    **kwargs,
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # MMPO
        if adv_estimator == "mmpo":
            adv_kwargs.pop("config", None)
            adv_kwargs["unbiased"] = kwargs.get("mmpo_unbiased", None)
            adv_kwargs["T"] = kwargs.get("mmpo_T", None)
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.config = config
        self._validate_dmpo_training_contract()
        validate_fb_training_contract(self.config)
        self._fb_state = None
        if self.config.algorithm.adv_estimator in {"fb_rloo", "fb_grpo"}:
            fb_config = self.config.algorithm.fb
            if not fb_config.enabled:
                raise ValueError("selected FB objective requires algorithm.fb.enabled=True")
            self._fb_state = FBI(
                self.config.algorithm.adv_estimator,
                k_train=fb_config.k_train,
                ema_beta=fb_config.failure_ema_beta,
                weight_mode=fb_config.get("weight_mode", "lagged"),
                weight_temperature=float(fb_config.get("weight_temperature", 1.0)),
                freeze_failure_updates=bool(fb_config.get("freeze_failure_updates", False)),
            )
    ...
    def _validate_dmpo_training_contract(self) -> None:
        cfg = self.config.algorithm.dmpo
        if not cfg.enabled:
            return
        actor = self.config.actor_rollout_ref.actor
        if self.config.algorithm.adv_estimator != "grpo":
            raise ValueError("DMPO requires algorithm.adv_estimator=grpo")
        if self.config.algorithm.use_kl_in_reward:
            raise ValueError("DMPO requires algorithm.use_kl_in_reward=False")
        if actor.policy_loss.loss_mode != "grpo_dmpo":
            raise ValueError("DMPO requires policy_loss.loss_mode=grpo_dmpo")
        if actor.use_kl_loss:
            raise ValueError("DMPO requires actor.use_kl_loss=False")
        if actor.ppo_epochs != 1:
            raise ValueError("DMPO requires actor.ppo_epochs=1")
        if actor.ppo_mini_batch_size != self.config.data.train_batch_size:
            raise ValueError("DMPO requires actor.ppo_mini_batch_size=data.train_batch_size")
        if getattr(self.config.data, "gen_batch_size", self.config.data.train_batch_size) != self.config.data.train_batch_size:
            raise ValueError("DMPO requires effective gen_batch_size=data.train_batch_size")
        if self.config.actor_rollout_ref.rollout.n != cfg.group_size:
            raise ValueError("DMPO rollout group size mismatch")
        rollout_correction = getattr(self.config.algorithm, "rollout_correction", None)
        if rollout_correction and rollout_correction.get("bypass_mode", False):
            raise ValueError("DMPO requires recomputed current-policy old_log_probs")

    def _validate_dmpo_response_batch(self, batch: DataProto) -> None:
        if self.config.algorithm.dmpo.enabled:
            expected = self.config.actor_rollout_ref.actor.ppo_mini_batch_size * self.config.actor_rollout_ref.rollout.n
            if batch.batch["response_mask"].shape[0] != expected:
                raise ValueError("DMPO requires one complete response minibatch per optimizer transaction")

    def _attach_dmpo_objective(self, batch: DataProto) -> DataProto:
        cfg = self.config.algorithm.dmpo
        if not cfg.enabled:
            return batch
        self._validate_dmpo_response_batch(batch)
        from recipe.mmpo.dmpo_objective import compute_dmpo_batch

        coefficient, group_mse = compute_dmpo_batch(
            batch.batch["token_level_rewards"],
            batch.batch["old_log_probs"],
            batch.batch["response_mask"],
            batch.non_tensor_batch["uid"],
            matching_weight=float(cfg.matching_weight),
            temperature=float(cfg.temperature),
            group_size=int(cfg.group_size),
        )
        batch.batch["dmpo_coefficients"] = coefficient
        batch.batch["dmpo_group_mse"] = group_mse
        return batch

    def _attach_fb_problem_ids(self, batch: DataProto) -> None:
        if self._fb_state is None:
            return
        if "data_source" not in batch.non_tensor_batch or "index" not in batch.non_tensor_batch:
            raise ValueError("FB requires data_source and source index metadata")
        batch.non_tensor_batch["fb_problem_id"] = stable_problem_ids(
            batch.non_tensor_batch["data_source"], batch.non_tensor_batch["index"]
        )

    def _compute_fb_advantage(self, batch: DataProto) -> DataProto:
        if self._fb_state is None:
            raise RuntimeError("FB trainer state is not initialized")
        rewards = batch.batch["token_level_rewards"].sum(dim=-1)
        advantages = self._fb_state.compute_advantages(
            rewards=rewards,
            response_mask=batch.batch["response_mask"],
            group_ids=batch.non_tensor_batch["uid"],
            problem_ids=batch.non_tensor_batch["fb_problem_id"],
        )
        batch.batch["advantages"] = advantages
        batch.batch["returns"] = advantages
        return batch

    def _commit_fb_update(self, actor_metrics: dict[str, Any]) -> None:
        if self._fb_state is None:
            return
        success = actor_metrics.get("actor/optimizer_step_succeeded")
        if isinstance(success, torch.Tensor) and success.numel() == 1:
            success = success.item()
        if not isinstance(success, (int, float)) or not math.isfinite(success) or success not in (0, 1):
            raise RuntimeError("FB actor output must report an all-rank binary optimizer success metric")
        self._fb_state.commit(bool(success))
        if self.config.algorithm.fb.get("mechanism_log", False):
            os.makedirs(self.config.trainer.default_local_dir, exist_ok=True)
            event = {"step": self.global_steps, "optimizer_step_succeeded": bool(success), **self._fb_state.last_metrics}
            with open(os.path.join(self.config.trainer.default_local_dir, "fb_mechanism.jsonl"), "a", encoding="utf-8") as handle:
                handle.write(json.dumps(event, allow_nan=False) + "\n")

    def _save_fb_state(self, checkpoint_root: str) -> None:
        if self._fb_state is not None:
            self._fb_state.save(checkpoint_root)

    def _load_fb_state(self, checkpoint_root: str) -> None:
        if self._fb_state is not None:
            self._fb_state.load(checkpoint_root)
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        reward_keys = set({"data_source", "reward_model", "extra_info", "uid", "fb_problem_id"}) & batch.non_tensor_batch.keys()
    ...
    @staticmethod
    def _fb_eval_seed(base_seed: int, candidate_index: int, identity: str) -> int:
        value = f"{base_seed}:{candidate_index}:{identity}".encode()
        return int.from_bytes(hashlib.blake2s(value, digest_size=4).digest(), "little") & 0x7FFFFFFF

    def _validate(self, merged: bool = False, candidate_index: int | None = None):
        data_source_lst = []
        ...
        for test_data in self.val_dataloader:
            ...
            if candidate_index is not None:
                test_batch.non_tensor_batch["uid"] = np.array(
                    [str(item["table2_id"]) for item in test_batch.non_tensor_batch["extra_info"]], dtype=object
                )
            ...
            if candidate_index is not None:
                identities = [str(item.get("table2_id")) for item in test_batch.non_tensor_batch["extra_info"]]
                base_seed = int(self.config.algorithm.fb.eval_seed)
                test_gen_batch.non_tensor_batch["__sampling_seed__"] = np.array(
                    [self._fb_eval_seed(base_seed, candidate_index, identity) for identity in identities],
                    dtype=np.int64,
                )
            ...
            if candidate_index is not None:
                ...
                reward_extra_infos_dict["table2_id"].extend(str(item["table2_id"]) for item in extra_infos)
                reward_extra_infos_dict["benchmark"].extend(str(item["benchmark"]) for item in extra_infos)
                reward_extra_infos_dict["problem_id"].extend(int(item["problem_id"]) for item in extra_infos)
                reward_extra_infos_dict["candidate_index"].extend([candidate_index] * len(test_batch))
                posterior_seed = self.config.algorithm.fb.get("eval_posterior_seed")
                if posterior_seed is not None:
                    reward_extra_infos_dict["posterior_seed"].extend([int(posterior_seed) + 1000 * candidate_index] * len(test_batch))
                reward_extra_infos_dict["decoding_seed"].extend(
                    self._fb_eval_seed(int(self.config.algorithm.fb.eval_seed), candidate_index, str(item["table2_id"]))
        ...
        if val_data_dir:
            if candidate_index is not None:
                val_data_dir = os.path.join(val_data_dir, f"draw_{candidate_index:02d}")
            self._dump_generations(
    ...
    def _validate_fb_posterior_draws(self, draw_count: int):
        if draw_count < 1 or self._fb_state is None:
            raise ValueError("FB posterior evaluation requires an enabled FB objective and at least one draw")
        if self.config.actor_rollout_ref.rollout.val_kwargs.n != 1:
            raise ValueError("FB posterior evaluation requires val_kwargs.n=1")
        results = []
        try:
            for candidate_index in range(draw_count):
                perturbation_id = f"fb-eval-{self.global_steps}-{candidate_index}"
                sampled = False
                try:
                    posterior_seed = self.config.algorithm.fb.get("eval_posterior_seed")
                    if posterior_seed is None:
                        self.actor_rollout_wg.sample_parameters(perturbation_id)
                    else:
                        self.actor_rollout_wg.sample_parameters(perturbation_id, noise_seed=int(posterior_seed) + 1000 * candidate_index)
                    sampled = True
                    self.checkpoint_manager.sleep_replicas()
                    self.checkpoint_manager.update_weights(self.global_steps)
                    results.append(self._validate(merged=True, candidate_index=candidate_index))
                finally:
                    if sampled:
                        self.checkpoint_manager.sleep_replicas()
                        self.actor_rollout_wg.restore_parameter_mean(perturbation_id, train=False)
        finally:
            self.checkpoint_manager.update_weights(self.global_steps)

        data_sources = np.concatenate(
            [source for result in results for source in result["data_sources"]], axis=0
        )
        sample_uids = [uid for result in results for uid in result["sample_uids"]]
        sample_turns = [turns for result in results for turns in result["sample_turns"]]
    ...
    def _save_checkpoint(self):
        from verl.utils.checkpoint.checkpoint_io import atomic_torch_save
        ...
        self._save_fb_state(local_global_step_folder)

        # save dataloader
        local_mkdir_safe(local_global_step_folder)
        dataloader_local_path = os.path.join(local_global_step_folder, "data.pt")
        dataloader_state_dict = self.train_dataloader.state_dict()
        sampler_generator = getattr(self.train_sampler, "generator", None)
        if sampler_generator is not None:
            dataloader_state_dict[_SAMPLER_GENERATOR_STATE] = sampler_generator.get_state()
    ...
    def _checkpoint_io_gate(self):
        if not self.config.algorithm.fb.get("checkpoint_io_gate", False):
            return
        trainer = self.config.trainer
        if (not self.config.algorithm.fb.enabled or trainer.get("val_only", False)
                or trainer.resume_mode != "resume_path" or trainer.del_local_ckpt_after_load):
            raise ValueError("checkpoint I/O gate requires non-destructive FB training resume")
        origin = trainer.resume_from_path
        destination = os.path.join(trainer.default_local_dir, f"global_step_{self.global_steps}")
        if os.path.realpath(origin) == os.path.realpath(destination):
            raise ValueError("checkpoint I/O gate must not overwrite its origin")
        self._save_checkpoint()
        try:
            trainer.resume_from_path = destination
            self._load_checkpoint()
        finally:
            trainer.resume_from_path = origin
        print(f"FB_CHECKPOINT_IO_GATE_PASSED step={self.global_steps} path={destination}", flush=True)
        ...
        if os.path.exists(dataloader_local_path):
            ...
            if at_epoch_boundary:
                ...
                if sampler_generator is None:
                    raise ValueError("epoch-boundary resume requires a stateful sampler generator")
                sampler_generator.set_state(_saved_sampler_generator_state(dataloader_state_dict))
            else:
                dataloader_state_dict.pop(_SAMPLER_GENERATOR_STATE, None)
                self.train_dataloader.load_state_dict(dataloader_state_dict)
        else:
            raise ValueError(f"resume checkpoint is missing dataloader state: {dataloader_local_path}")
        self._load_fb_state(global_step_folder)
    ...
    def _update_actor(self, batch: DataProto) -> DataProto:
        ...
        if self._fb_state is not None:
            perturbation_id = batch.meta_info.get("fb_perturbation_id")
            if not isinstance(perturbation_id, str) or not perturbation_id:
                raise RuntimeError("FB actor update requires a logical perturbation ID")
            tu.assign_non_tensor(batch_td, fb_perturbation_id=perturbation_id)
    ...
    def fit(self):
        ...
        if self.config.trainer.get("val_before_train", True):
            posterior_draws = int(self.config.algorithm.fb.get("eval_posterior_draws", 0))
            val_metrics = (
                self._validate_fb_posterior_draws(posterior_draws) if posterior_draws else self._validate()
            )
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                self._attach_fb_problem_ids(batch)
                ...
                if self._fb_state is not None:
                    perturbation_id = self._fb_state.next_perturbation_id(self.global_steps)
                    batch.meta_info["fb_perturbation_id"] = perturbation_id
                    gen_batch.meta_info["fb_perturbation_id"] = perturbation_id
                    with marked_timer("sample_fb_parameters", timing_raw, color="red"):
                        self.actor_rollout_wg.sample_parameters(perturbation_id)
                        self.checkpoint_manager.sleep_replicas()
                        self.checkpoint_manager.update_weights(self.global_steps)
                rollout_n = self.config.actor_rollout_ref.rollout.n
                gen_batch_output = gen_batch.repeat(repeat_times=rollout_n, interleave=True)
                if self._fb_state is not None and self.config.algorithm.fb.get("train_decoding_seed") is not None:
                    gen_batch_output.non_tensor_batch["__sampling_seed__"] = training_decoding_seeds(
                        batch.non_tensor_batch["fb_problem_id"], self.global_steps, rollout_n,
                        int(self.config.algorithm.fb.train_decoding_seed),
                    )
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if self._fb_state is not None:
                            batch = self._compute_fb_advantage(batch)
                        else:
                            batch = compute_advantage(
                                ...
                                mmpo_T=self.config.algorithm.mmpo.T if self.config.algorithm.adv_estimator == "mmpo" else None,
                                mmpo_unbiased=self.config.algorithm.mmpo.unbiased if self.config.algorithm.adv_estimator == "mmpo" else None,
                            )
                        batch = self._attach_dmpo_objective(batch)
                    ...
                    if self.config.trainer.critic_warmup > self.global_steps:
                        ...
                        self._commit_fb_update(actor_output_metrics)
