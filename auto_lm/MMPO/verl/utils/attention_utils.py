...
def _get_attention_functions() -> tuple[Callable, Callable, Callable, Callable]:
    ...
    if is_torch_npu_available(check_device=False):
        ...
        try:
            ...
            if error.name not in {"flash_attn", "flash_attn.bert_padding"}:
                raise
            try:
                from transformers.modeling_flash_attention_utils import _pad_input as pad_input, _unpad_input as unpad_input
            except ImportError:
                from transformers.modeling_flash_attention_utils import _fa3_pad_input as pad_input, _fa3_unpad_input as unpad_input
            from einops import rearrange

            index_first_axis = lambda tensor, indices: tensor[indices]
