...
from verl.workers.engine.ivon_contract import inject_ivon_covariance
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
            ...
            if self.should_load_optimizer:
                ...
                covariance_path = os.environ.get("VERL_IVON_COVARIANCE_FROM_PATH")
                if covariance_path:
                    donor_path = os.path.join(
                        covariance_path, f"optim_world_size_{self.world_size}_rank_{self.rank}.pt"
                    )
                    donor_state_dict = torch.load(copy_to_local(donor_path), weights_only=False)
                    inject_ivon_covariance(
                        self.optimizer,
                        donor_state_dict,
                        require_noop=os.path.realpath(donor_path) == os.path.realpath(remote_optim_path),
                    )
                    del donor_state_dict
                    log_with_rank(
                        f"Injected IVON covariance only from {donor_path}", rank=self.rank, logger=logger
                    )
