...
class FSDPOptimizerConfig(OptimizerConfig):
    ...
    ivon_ess: Optional[float] = None
    ivon_hess_init: Optional[float] = None
    ivon_clip_radius: Optional[float] = None
    ivon_state_device: Optional[str] = None
    ivon_noise_seed: Optional[int] = None
...
def build_optimizer(parameters, config: FSDPOptimizerConfig):
    ...
    elif optimizer_name_lower == "ivon":
        required = {
            "ivon_ess": config.ivon_ess,
            "ivon_hess_init": config.ivon_hess_init,
            "ivon_clip_radius": config.ivon_clip_radius,
            "ivon_state_device": config.ivon_state_device,
        }
        missing = [key for key, value in required.items() if value is None]
        if missing:
            raise ValueError(f"IVON requires explicit configuration fields: {', '.join(missing)}")
        optimizer_args.update(
            beta1=config.betas[0],
            beta2=config.betas[1],
            ess=config.ivon_ess,
            hess_init=config.ivon_hess_init,
            clip_radius=config.ivon_clip_radius,
            state_device=config.ivon_state_device,
            noise_seed=config.ivon_noise_seed,
            mc_samples=1,
            train_mc_samples=1,
            sync=False,
        )
