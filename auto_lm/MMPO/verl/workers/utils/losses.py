...
def ppo_loss(config: ActorConfig, model_output, data: TensorDict, dp_group=None):
    ...
    if loss_mode in {"fb_rloo", "grpo_dmpo"}:
        # Ray workers import this module independently of the MMPO driver.
        import recipe.mmpo.core_algos  # noqa: F401

    # select fields and convert to padded tensor
    fields = ["response_mask", "old_log_probs", "advantages"]
    if loss_mode == "grpo_dmpo":
        fields.extend(["dmpo_coefficients", "dmpo_group_mse"])
    ...
    if loss_mode == "grpo_dmpo":
        policy_loss_kwargs["dmpo_coefficients"] = data["dmpo_coefficients"]
        policy_loss_kwargs["dmpo_group_mse"] = data["dmpo_group_mse"]
    pg_loss, pg_metrics = policy_loss_fn(**policy_loss_kwargs)

    # AggregationType.MEAN for pg metrics: assumes policy_loss_fn normalizes by local_bsz/local_tokens
    # Ex: in compute_policy_loss_vanilla, pg_metrics are pg_clipfrac, ppo_kl, pg_clipfrac_lower
    pg_metrics = Metric.from_dict(pg_metrics, aggregation=AggregationType.MEAN)
    if "actor/dmpo_surrogate" in pg_metrics:
        pg_metrics["actor/dmpo_surrogate"].aggregation = AggregationType.SUM
