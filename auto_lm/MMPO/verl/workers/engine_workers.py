...
class TrainingWorker(Worker, DistProfilerExtension):
    ...
    def sample_parameters(self, perturbation_id: str, noise_seed: int | None = None) -> None:
        self.engine.sample_parameters(perturbation_id, noise_seed=noise_seed)

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def restore_parameter_mean(self, perturbation_id: str, train: bool = False) -> None:
        self.engine.restore_parameter_mean(perturbation_id, train=train)
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def sample_parameters(self, perturbation_id: str, noise_seed: int | None = None) -> None:
        if not self._is_actor:
            raise RuntimeError("parameter sampling requires an actor worker")
        self.actor.sample_parameters(perturbation_id, noise_seed=noise_seed)

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def restore_parameter_mean(self, perturbation_id: str, train: bool = False) -> None:
        if not self._is_actor:
            raise RuntimeError("parameter restoration requires an actor worker")
        self.actor.restore_parameter_mean(perturbation_id, train=train)
