...
class BaseEngine:
    ...
    def sample_parameters(self, perturbation_id: str, noise_seed: int | None = None) -> None:
        raise RuntimeError("this engine does not support IVON parameter sampling")

    def assert_active_perturbation(self, perturbation_id: str) -> None:
        raise RuntimeError("this engine does not support IVON perturbation checks")

    def restore_parameter_mean(self, perturbation_id: str, *, train: bool) -> None:
        raise RuntimeError("this engine does not support IVON mean restoration")
    ...
    def train_batch(self, data: TensorDict, loss_function: Callable) -> Any:
        ...
        if self.uses_ivon_optimizer and perturbation_id is None:
            raise RuntimeError("IVON training requires an FB logical perturbation ID")
        if perturbation_id is not None and not self.uses_ivon_optimizer:
            raise RuntimeError("an FB logical perturbation ID requires the IVON optimizer")
        ...
        try:
            ...
            if perturbation_id is not None:
                self.restore_parameter_mean(perturbation_id, train=False)
            raise
        if perturbation_id is not None:
            self.restore_parameter_mean(perturbation_id, train=True)
        step_result = self.optimizer_step()
        if isinstance(step_result, tuple):
            grad_norm, optimizer_step_succeeded = step_result
        else:
            grad_norm, optimizer_step_succeeded = step_result, True
        if self.is_mp_src_rank_with_outputs():
            assert "grad_norm" not in outputs["metrics"]
            outputs["metrics"]["grad_norm"] = grad_norm
            outputs["metrics"]["optimizer_step_succeeded"] = float(optimizer_step_succeeded)
        return outputs
