...
from ..ivon_contract import assert_ivon_perturbation, restore_ivon_parameter_mean, sample_ivon_parameters
...
def _apply_resume_lr_override(optimizer, lr_scheduler, value):
    learning_rate = float(value)
    if not 0 <= learning_rate < float("inf"):
        raise ValueError("resume learning-rate override must be nonnegative and finite")
    for group in optimizer.param_groups:
        group["lr"] = learning_rate
        group["initial_lr"] = learning_rate
    if lr_scheduler is not None:
        lr_scheduler.base_lrs = [learning_rate] * len(optimizer.param_groups)
        lr_scheduler._last_lr = [learning_rate] * len(optimizer.param_groups)
...
class FSDPEngine(BaseEngine):
    ...
    @property
    def uses_ivon_optimizer(self) -> bool:
        return self.optimizer_config.optimizer.lower() == "ivon"

    def sample_parameters(self, perturbation_id: str, noise_seed: int | None = None) -> None:
        if not self.uses_ivon_optimizer:
            raise RuntimeError("parameter sampling requires optimizer=IVON")
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.module)
        if self._is_offload_optimizer:
            load_fsdp_optimizer(self.optimizer, get_device_name())
        sample_ivon_parameters(self.optimizer, perturbation_id, noise_seed=noise_seed)
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.module)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(self.optimizer)

    def assert_active_perturbation(self, perturbation_id: str) -> None:
        assert_ivon_perturbation(self.optimizer, perturbation_id)

    def restore_parameter_mean(self, perturbation_id: str, *, train: bool) -> None:
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.module)
        if self._is_offload_optimizer:
            load_fsdp_optimizer(self.optimizer, get_device_name())
        restore_ivon_parameter_mean(self.optimizer, perturbation_id, train=train)
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.module)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(self.optimizer)
    ...
    def forward_backward_batch(self, data: TensorDict, loss_function: Callable, forward_only=False) -> list[TensorDict]:
        ...
        perturbation_id = tu.get(data, key="fb_perturbation_id", default=None)
            ...
            with ctx:
                ...
                if not forward_only:
                    if perturbation_id is not None:
                        self.assert_active_perturbation(perturbation_id)
    ...
    def optimizer_step(self):
        ...
        max_norm = float("inf") if self.uses_ivon_optimizer else self.optimizer_config.clip_grad
        if isinstance(self.module, FSDP):
            grad_norm = self.module.clip_grad_norm_(max_norm)
        elif isinstance(self.module, FSDPModule):
            grad_norm = fsdp2_clip_grad_norm_(self.module.parameters(), max_norm=max_norm)
        else:
            grad_norm = torch.nn.utils.clip_grad_norm_(self.module.parameters(), max_norm=max_norm)
        ...
        return grad_norm.item(), optimizer_step_succeeded
    ...
    def load_checkpoint(
        ...
        resume_lr_override = os.environ.get("VERL_RESUME_LR_OVERRIDE")
        if resume_lr_override:
            _apply_resume_lr_override(self.optimizer, self.lr_scheduler, resume_lr_override)
            if self.rank == 0:
                print(f"RESUME_LR_OVERRIDE_APPLIED lr={float(resume_lr_override)}", flush=True)
